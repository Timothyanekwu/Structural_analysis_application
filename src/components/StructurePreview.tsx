...
export default function StructurePreview({ members, highlightNode }: StructurePreviewProps) {
  ...
  const renderSupport = (type: SupportType, x: number, y: number, key: string) => {
    const px = scaleX(x);
    const py = scaleY(y);
    ...
    if (type === "Roller") {
      return (
        <g key={key}>
          <circle cx={px} cy={py + 5} r="5" fill="none" stroke="var(--support)" strokeWidth="2" />
          <line x1={px - 10} y1={py + 10} x2={px + 10} y2={py + 10} stroke="var(--support)" strokeWidth="2" />
        </g>
      );
    }
    return null;
        ...
        {members.map((member, i) => {
          ...
          return (
            <g key={i}>
              ...
              {/* Supports */}
              {renderSupport(member.supports.start, member.startNode.x, member.startNode.y, `support-start-${i}`)}
              ...
              {member.loads.map((load, j) => {
                ...
                if (load.type === "Point") {
                  const mLen = Math.sqrt(Math.pow(member.endNode.x - member.startNode.x, 2) + Math.pow(member.endNode.y - member.startNode.y, 2)) || 1;
                  ...
                  return (
                    <g key={`load-${i}-${j}`} className="text-[var(--accent)]">
                      <line 
                        x1={x - dx_load} y1={y - dy_load} 
                        x2={bx} y2={by} 
                        stroke="currentColor" strokeWidth="2" 
                        className="drop-shadow-[0_0_3px_var(--accent-glow)]" 
                      />
                      <polygon points={`${x},${y} ${p1x},${p1y} ${p2x},${p2y}`} fill="currentColor" />
                      <text 
                        x={x - dx_load * 1.3} y={y - dy_load * 1.3} 
                        textAnchor="middle" fill="currentColor" fontSize="10" fontWeight="bold" dominantBaseline="middle"
                      >
                        {loadVal}kN
                      </text>
                    </g>
                ...
                if (load.type === "UDL") {
                  ...
                  const tE = (Number(load.position || 0) + Number(load.span || 0)) / mLen;
                  ...
                  const sSpan = Math.sqrt(Math.pow(xE - xS, 2) + Math.pow(yE - yS, 2)) || 1;
                  ...
                  return (
                    <g key={`load-${i}-${j}`}>
                       {/* UDL Base Line */}
                       <line 
                         x1={xS - dx_load} y1={yS - dy_load} 
                         x2={xE - dx_load} y2={yE - dy_load} 
                       ...
                       {Array.from({ length: density }).map((_, k) => {
                         ...
                         return (
                           <g key={k} className="text-[var(--accent)]">
                             <line 
                               x1={px - dx_load} y1={py - dy_load} 
                               x2={bx} y2={by} 
                               stroke="currentColor" strokeWidth="1" 
                             />
                             <polygon points={`${px},${py} ${p1x},${p1y} ${p2x},${p2y}`} fill="currentColor" />
                           </g>
                         );
                       })}
                       {/* Label */}
                       <g transform={`translate(${(xS + xE) / 2 - dx_load * 1.6}, ${(yS + yE) / 2 - dy_load * 1.6})`}>
                        <rect x="-25" y="-12" width="50" height="15" rx="4" fill="#000" />
                        <text textAnchor="middle" fill="var(--accent)" fontSize="8" fontWeight="bold" dominantBaseline="middle">{loadVal}kN/m</text>
                       </g>
                    </g>
                  );
                }
                ...
                if (load.type === "VDL") {
                  ...
                  return (
                    <g key={`load-${i}-${j}`}>
                       ...
                       <line 
                         x1={x1 + (x2 - x1) * (highPos / mLen) - Math.cos(angle) * arrowLengthIdx} 
                         y1={y1 + (y2 - y1) * (highPos / mLen) - Math.sin(angle) * arrowLengthIdx} 
                       ...
                       {/* Label at Peak */}
                       <g transform={`translate(${x1 + (x2 - x1) * (highPos / mLen) - Math.cos(angle) * arrowLengthIdx * 1.5}, ${y1 + (y2 - y1) * (highPos / mLen) - Math.sin(angle) * arrowLengthIdx * 1.5})`}>
                        <rect x="-25" y="-12" width="50" height="15" rx="4" fill="#000" />
                        <text textAnchor="middle" fill="var(--accent)" fontSize="8" fontWeight="bold" dominantBaseline="middle">{highVal.toFixed(1)}kN/m</text>
                       </g>
                    </g>
                  );
                }
              ...
              {/* Node Indicators */}
              <circle cx={x1} cy={y1} r="4" fill="#050505" stroke="var(--primary)" strokeWidth="2" />
              <circle cx={x2} cy={y2} r="4" fill="#050505" stroke="var(--primary)" strokeWidth="2" />
            </g>
          );
        })}
